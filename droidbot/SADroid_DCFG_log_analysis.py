...
def find_evasion(app_logdir, conn):
    ...
    for method_dir in os.listdir(app_logdir):
        ...
        real_device_log_file = os.path.join(app_logdir, method_dir, 'real_device_log.txt')
        emu_device_log_file = os.path.join(app_logdir, method_dir, 'emulator_device_log.txt')
        ...
        for i, real_block in enumerate(real_blocklist[:-1]):
            ...
            # input(f'Find evasion:{real_block} belong to method:{method_sign}, hash:{method_dir}')
            evasion.append(real_block)
    
    with open(os.path.join(app_logdir, 'evasion.json'), 'w') as f:
        for item in evasion:
            f.write(f"{item}\n")
            
def split_log_lines(log_lines, is_emu, app_hash, conn, app_logdir):
    #用method hash作為區分把log_lines分成多個資料夾內的檔案 每個method sign對應一個資料夾
    discovered_method = {}
    method_hash_map = {}
    ...
    for line in log_lines:
        ...
        discovered_method[method_sign].append(line_info.split(', [')[0])
        
    # input(f'method_hash_map:{method_hash_map}')
    for method_hash, method_sign in method_hash_map.items():
        ...
        if method_sign in discovered_method:
            with open(log_file_path, 'w+') as log_file:
                for line_info in discovered_method[method_sign]:
...
if __name__ == "__main__":
    ...
    for app_name in tqdm(os.listdir(logcat_dir)):
        ...
        #     split_log_lines(log_lines, is_emu, app_hash, conn, app_logdir)
